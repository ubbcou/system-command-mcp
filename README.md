...
- 程序和执行环境在启动时生成快照；自动发现环境或 Manifest 配置的变化都需要重启 Server 才会生效。若 Manifest 设置可选的 `nodeResolution: { "enabled": true, "installationRoots": ["~/…"] }`，`node` 仍是一个 Logical Program，但每次执行会从 cwd 向上（止于物理 Root）读取最近声明：`package.json` 的 `devEngines.runtime(name: node)`、`volta.node`、`.nvmrc`、`.node-version`、`engines.node`（按此优先级）。启动时发现并冻结安装版本；执行时仅重新读取声明。有效 semver/range 选择满足范围的最高安装版本；无声明使用 Manifest 的 node，缺失匹配报 `NODE_VERSION_UNAVAILABLE`，无效要求报 `NODE_VERSION_REQUIREMENT_INVALID`。`lts/*`、`node`、`stable` 和 `packageManager` 不参与解析。结果的 `programSelection` 可审计实际选择，`system_environment.programs.node.resolver` 显示安装快照。动态 `node` 子进程 PATH 会优先其目录；逻辑 `npm` 仍是已配置 wrapper。
