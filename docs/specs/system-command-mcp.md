...
22. As a Windows user, I want PATHEXT behavior to support `.EXE`, `.CMD`, `.BAT`, and configured extensions, so that npm and pnpm wrappers work.
23. As a user, I want the contract to acknowledge Platform Wrappers without accepting shell command strings, so that Windows support is accurate rather than misleading.
...
76. As a user, I want each Execution Request to own its full Process Tree, so that timeout and cancellation do not leave descendant workers running.
...
83. As a Windows user, I want a validated Lifecycle Adapter that cleans `.exe` and `.cmd` descendant trees, so that package-manager and test-runner timeouts are reliable.
...
25. The public direct-program contract remains `program + args[]`; the Server never accepts a caller-controlled shell command string. Platform Wrappers may require runtime-specific launching, particularly for Windows `.cmd` and `.bat` files.
...
28. Every Execution Request owns a Process Tree. Lifecycle Adapters first request graceful tree termination, wait a configurable Grace Period, force termination, then wait a bounded final interval.
...
31. Termination Outcome reports whether termination was requested, its reason, whether force was used, whether cleanup was confirmed, and any cleanup error. Normal completion has `treeCleaned: null`.
...
10. Process Tree tests use a shared fixture that creates descendants through native executables and wrappers. The contract asserts graceful termination, force escalation, cleanup reporting, bounded return, and no surviving tracked descendants.
