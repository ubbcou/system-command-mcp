...
82. As a user, I want normal completion to report whether contained members needed cleanup, so that a root exit does not leave owned descendants running.
...
9. The Program Manifest has strict version-specific contracts. Manifest v1 is frozen as the transition format and permits the existing optional `nodeResolution` field but rejects `projectNode`. Manifest v2 rejects `nodeResolution` and permits optional `projectNode`. Both versions contain execution defaults, Program definitions, common search paths, optional PATHEXT, Environment Layers, platform overrides, and probes; unknown fields and empty Candidate arrays are invalid at the top level and inside every platform object.
10. Platform overrides shallow-merge ordinary objects by key and replace arrays. Program definitions merge by Logical Program name and then by Program field. `enabled: false` disables an Optional Program on a platform; disabling a Core Program is invalid. In v1, `nodeResolution` may be overridden by platform. In v2, `projectNode` is a full override: an active-platform object replaces the top-level object rather than merging its fields.
...
25. The public direct-program contract remains `program + args[]`; the Server never accepts a caller-controlled shell command string. Native executables receive literal arguments. Windows `.cmd` and `.bat` Platform Wrappers use a narrowed cmd.exe-reparsed capability: `%`, `!`, `&`, `|`, `<`, `>`, and `^` arguments are rejected because `%PATH%` can expand/split, `!VAR!` depends on delayed expansion, and metacharacters can be parsed before the wrapper; accepted wrapper arguments do not receive the native arbitrary literal-argument guarantee.
...
30. Windows lifecycle uses a per-request Job Object with breakaway disabled and `JOB_OBJECT_LIMIT_KILL_ON_JOB_CLOSE` crash cleanup; `child.kill()` is abrupt on Windows and never represents graceful tree termination. Forced `taskkill /T /F` is an unconfirmed fallback whose observed status/output are diagnostics only (including any local `128` observation), not a portable semantic contract. Unix uses an isolated process group and group signals, validated on macOS with the shared descendant-process fixture.
31. Termination Outcome reports whether termination was requested, its reason, whether force was used, whether contained-member cleanup was independently confirmed, and any cleanup error. `treeCleaned: true` is permitted only after that confirmation; containment or confirmation failure—including the Windows post-spawn/pre-assignment race, assignment rejection, an escaped Unix `setsid()` group, or the taskkill traversal snapshot—returns `treeCleaned: false` plus diagnostics. Natural root completion has `reason: null`; `treeCleaned` is null only when no cleanup was needed, otherwise it is the boolean cleanup result and `forceUsed` truthfully records escalation.
...
34. Inline Output defaults to 256 KiB head and 768 KiB tail per stream, with a configurable total and an 8 MiB hard maximum. Its head must be positive and no larger than its total. `text` remains and contains the head/tail projection with an omission marker when truncated.
...
39. Default Artifact retention is 24 hours, with a 24-hour hard maximum; total cache maximum is 1 GiB, and execution maximum is 100 MiB per stream. Reaching a persistence limit does not terminate or alter the process result.
...
47. `doctor` performs static validation of Roots and all effective runtime limits by default. `doctor --execute` runs only explicit Execution Probes, defaults to Core Programs, and uses accepted exit-code sets. Probes never run during Server startup. Every unresolved Optional Program is reported as an `optional-unavailable` warning with its Program Candidate names.
48. `doctor` exits zero for runnable configuration even with Optional warnings, one for an unusable configuration/environment, and two for invalid CLI invocation. Configured `doctor` and `print-config` invocations require at least one Root.
...
53. Manifest v2 `projectNode` is the bounded exception to static program selection. It requires non-empty `enabledRoots` and `installationRoots` arrays plus an exact `defaultVersion`; optional `whenNoSelector` is `default-version` (default) or `active-manager`, and active-manager requires `activeManagerLinks` with exactly one explicit link; the array shape exists only for platform replacement configuration. Startup constructs an immutable Program Variant Set from recognized Node-manager layouts under the installation roots. Discovery accepts only exact semver version-directory metadata plus canonical native executables and optional canonical paired npm/npx CLI files. It does not execute binaries or managers, consult manager defaults or aliases, install software, access the network, or mutate global state. `defaultVersion` must name an exact startup Variant.
54. Each `projectNode.enabledRoots` entry must resolve inside Host Authorization and is the sole upward lookup boundary for requests inside it. Eligible requests walk from canonical cwd through the containing enabled Root inclusive. VCS markers, repository roots, package workspaces, and monorepo metadata neither stop nor widen lookup. Requests outside all enabled Roots use static Program registrations without Project Node Resolution.
55. Supported Project Node Declarations are re-read per eligible `node`, `npm`, or `npx` Execution Request. This is request input evaluation, not Manifest watching, PATH watching, installation-root watching, dynamic Program registration, or `tools/list_changed`; those remain startup snapshots and require restart to change.
56. Exact-selection declarations are `package.json#devEngines.runtime` for Node, `package.json#volta.node`, `.nvmrc`, and `.node-version`. Each must be an exact semver pin; ranges and aliases/defaults such as `default`, `node`, `stable`, or `lts/*` are invalid. The nearest directory containing exact declarations wins, and all exact declarations there must normalize to one version or fail with a conflict. `package.json#engines.node` never selects a version: every occurrence from cwd through the enabled Root is a compatibility range that the exact selection, or configured no-selector selection, must satisfy. With `default-version`, the exact `defaultVersion` is selected. With `active-manager`, the single configured link is authoritative on every request; selection linearizes at its captured `readlink` value, resolves an absolute or link-relative captured target, and canonicalizes that target before matching one startup Variant directory or executable. A concurrent repoint affects the next request, while selected target and Variant identities are revalidated before launch. Missing, broken, non-link, or unmatched state immediately fails with `PROJECT_NODE_ACTIVE_VERSION_UNAVAILABLE` without consulting another link or the default. A switch to a snapshotted Variant takes effect without restart, while a newly installed Variant requires restart. Missing snapshot pins and incompatible engines fail without fallback, installation, or networking.
57. A project-selected or Manifest-default `node` prepends its Variant directory only to that child's PATH. Registered `npm` and `npx` must pair with the same selected Variant and run its canonical `node_modules/npm/bin/npm-cli.js` or `npx-cli.js` through that Variant's Node executable; absence of the paired CLI fails statically. Static npm/npx wrappers remain in use only outside Project Node Boundaries. pnpm, yarn, and other package managers are not redirected.
58. Public Schema may make deliberate compatibility corrections before 1.0. After 1.0, minor releases only add optional fields or otherwise preserve semantics.
59. The current source is expected to become an Authoritative Package separate from the Skillshare Integration. Repository ownership, package scope, license, and other non-implementation governance are intentionally deferred.
...
5. Manifest contract tests cover strict v1/v2 `oneOf` validation, v1 `nodeResolution`/v2 `projectNode` exclusivity at the top level and in platforms, unknown fields, empty Candidates, shallow object merge, array replacement, v2 full `projectNode` platform override, platform enable/disable semantics, relative/home/PATH Candidate resolution, Environment References, Core/Optional behavior, deterministic search order, and Shadowed Candidate diagnostics.
...
10. Process Tree tests use a shared fixture that creates descendants through native executables and wrappers. The contract asserts graceful termination, force escalation, independently confirmed contained-member cleanup, bounded return, and no surviving tracked descendants; Unix `setsid()`, Windows Job-assignment-race/breakaway attempts, and taskkill traversal snapshots are explicit escape tests that must not yield `treeCleaned: true` for an unconfirmed unit.
11. Windows Lifecycle Adapter tests run under Node 20 and the latest LTS and cover per-request Job Object assignment with breakaway disabled, `KILL_ON_JOB_CLOSE` crash cleanup, `.exe`, `.cmd`, grandchildren, permission failure, forced termination, and unconfirmed `taskkill` fallback diagnostics without relying on any particular taskkill exit code.
...
23. Project Node tests cover enabled Roots as the sole boundaries, requests outside them using static registrations, absence of VCS/workspace heuristics, per-request declaration re-read without Manifest/PATH/installation watching, exact pins, Manifest `defaultVersion`, explicit active-manager link switching without restart, broken/non-link/unmatched active states, exact-selector precedence, compatible and conflicting exact declarations, ancestor `engines.node` compatibility, invalid ranges/defaults/aliases, unsatisfied snapshot requirements, no install/network/global mutation, child-only PATH precedence, and npm/npx same-Variant pairing or static failure.
24. Node.js tests cover the minimum supported LTS line and the latest LTS. The hard platform-host matrix is Windows x64 plus Codex/DSH and macOS arm64 plus Codex/DSH.
25. Tests must verify that stderr logging never contaminates MCP stdout and that arguments, Execution Input, environment values, and output content are absent from Server logs and Artifact metadata.
...
- Dynamic Program registration, Manifest watching, PATH watching, installation-root watching, or `tools/list_changed` behavior during one Server lifetime. Re-reading an eligible project's Node declarations for one Execution Request is request input evaluation, not an exception to this exclusion.
