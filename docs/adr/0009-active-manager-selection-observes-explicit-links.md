...
For each eligible Node, npm, or npx request without an exact selector, the configured link is authoritative. Selection linearizes at one `readlink` result: the Runtime captures that absolute or link-relative target, resolves the captured value, and only then canonicalizes it. If the manager repoints the link concurrently, the current request keeps the captured target and the next request observes the new target. The canonical target must exactly match a version directory or Node executable in the immutable startup Program Variant Set, whose directory and executable identities are revalidated before launch. Missing, broken, ordinary directories/files, and unmatched state immediately fail with `PROJECT_NODE_ACTIVE_VERSION_UNAVAILABLE`; no alternative link or fixed default is consulted. Project `engines.node` constraints still apply.
