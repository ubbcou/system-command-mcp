...
import { codexSnippet, doctor, dshSnippet, EXIT, readManifest, runtimeOptions, writeManifestTemplate, type ManagementOptions } from "./management.js";
import { createServer } from "./server.js";

type Command = "serve" | "init" | "doctor" | "print-config";
interface Parsed { command: Command; target?: "codex" | "dsh"; options: ManagementOptions; probe: boolean; initPath?: string; legacy: boolean; }
...
export function parseCli(argv: readonly string[]): Parsed {
  ...
  let probe = false; let target: "codex" | "dsh" | undefined; let initPath: string | undefined;
  ...
  for (; index < argv.length; index++) {
    ...
    if (flag === "--probe" && command === "doctor") { probe = true; continue; }
  ...
  return { command, target, options, probe, initPath, legacy };
}
...
export async function main(argv = process.argv.slice(2)): Promise<void> {
  ...
  if (parsed.command === "init") {
    ...
    await writeManifestTemplate(path);
  ...
  if (parsed.command === "doctor") {
    const result = await doctor(parsed.options, parsed.probe); console.error(`doctor: ${result.message}`); return;
  }
