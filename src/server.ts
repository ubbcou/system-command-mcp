...
function environmentValue(snapshot: RuntimeEnvironment): Record<string, unknown> {
  return {
    ...
    programs: Object.fromEntries(Object.entries(snapshot.programs).map(([name, program]) => [name, { executable: program.executable, declaredCandidate: program.declaredCandidate, kind: program.kind, argumentSemantics: program.argumentSemantics, ...(program.resolver ? { resolver: program.resolver } : {}) }])),
  };
}
